...
describe KingDta::SumupDtazv do
  ...
  it "should create a T-block with customer bill text in it in EUR" do
    sumup_dtazv = KingDta::SumupDtazv.new('EUR', Date.parse('2011-08-28'))
    sumup_dtazv.account = KingDta::Account.new sender_opts
  ...
  it "should create a T-block with customer bill text in it in EUR" do
    sumup_dtazv = KingDta::SumupDtazv.new('GBP', Date.parse('2011-08-28'))
    sumup_dtazv.account = KingDta::Account.new sender_opts
    @booking.customer_bill_text = "A"*27
    sumup_dtazv.add_t(@booking)
    str = sumup_dtazv.dta_string
    out = "0768T37050299GBP132604963411082800000000   0000000000COBADEFF366                                                                                                                                               DE DALAI LAMA                                                            BUSH-AVENUE 55                     445555 INDIA                                                                                             /PL10105013151000002308622378      GBP00000000000220250                                                                                                                                            00000000                         0000AAAAAAAAAAAAAAAAAAAAAAAAAAA                                   0                                                   00"
    str.should == out
  end
end
